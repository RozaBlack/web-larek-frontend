...
Конструктор класса принимает инстант брокера событий (но это не точно)\
...
#### Класс ModalWithInfo
Расширяет класс Modal. Предназначен для реализации модального окна подтверждения успешности заказа.\
Поля класса:
...
#### Класс ModalWithForm
Расширяет класс Modal. Предназначен для реализации модального окна с формой содержащей поля ввода. При сабмите инициирует событие передавая в него объект с данными из полей ввода формы и открывает следующующее модальное окно. При изменении данных в полях ввода инициирует событие изменения данных. Предоставляет методы для отображения ошибок и управления активностью кнопки сохранения.\
Конструктор класса передается DOM элемент темплейта.\
...
#### Класс ModelWithBasket
Расширяет класс Modal. Отвечает за отображение корзины с товарами. Класс отображает нумерованный список с элементами в корзине, а также отображает их суммарную стоимость.\
...
Методы:
- getTotalSum(productList: IProduct[]) - получает и сохраняет суммарную стоимость всех товаров в корзине
- getNumberOfProducts(productList: IProduct[]) - получает и сохраняет количество элементов в корзине
...
Расширяет классы Modal и Card. Отвечает за отображение модального окна с карточкой товара.\
В конструктор класса передается DOM элемент темплейта. В классе устанавливаются слушатели на все интерактивные элементы, в результате взаимодействия с которыми пользователя генерируются соответствующие события.\
Поля класса содержат элементы разметки элементов карточки. Конструктор, кроме темплейта принимает экземпляр `EventEmitter` для инициации событий.\
Методы:
